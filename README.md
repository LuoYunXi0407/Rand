...
- 💡如果你下载的是 `.zip` 结尾的，你可以直接跳到 [🖥️创建桌面快捷方式](https://github.com/LuoYunXi0407/Rand#%EF%B8%8F-%E5%88%9B%E5%BB%BA%E6%A1%8C%E9%9D%A2%E5%BF%AB%E6%8D%B7%E6%96%B9%E5%BC%8F) 部分
- 💡如果你下载的是 `.exe` 结尾的可执行文件，你可能需要 [下载并安装 .Net 9.0 桌面运行时](https://github.com/LuoYunXi0407/Rand#%E4%B8%8B%E8%BD%BD%E5%B9%B6%E5%AE%89%E8%A3%85-net-90-%E6%A1%8C%E9%9D%A2%E8%BF%90%E8%A1%8C%E6%97%B6)

#### 下载并安装 .Net 9.0 桌面运行时
![image](https://github.com/user-attachments/assets/53479542-8cc4-4f3f-a2c6-39b4079fdae9)

请访问 [这个网页](https://dotnet.microsoft.com/zh-cn/download/dotnet/9.0)，在「.NET 桌面运行时」部分，根据操作系统是 64 位或者 32 位下载并安装。
- 💡如果你不知道你的操作系统是 64 位还是 32 位，并且不知道应该下载哪一个，请下载并安装 `x64`，除非你的电脑极其老旧，并且在 [前面的下载步骤](https://github.com/LuoYunXi0407/Rand#%EF%B8%8F-%E4%B8%8B%E8%BD%BD) 下载的是 `win_x86.exe` 结尾的可执行文件，那应该下载并安装 `x86`

### 🖥️ 创建桌面快捷方式
- 💡建议把下载的文件放到硬盘某个地方（例如 D 盘的某个文件夹内），而不是桌面
- 💡如果你下载的是 `.exe` 结尾的可执行文件，你可以直接 `右键-（显示更多选项）-发送到-桌面快捷方式` 来创建快捷方式
  
![image](https://github.com/user-attachments/assets/a28798ad-85fb-4bab-88c2-5f495e0a7ec3)



- 💡如果你下载的是 `.zip` 结尾的可执行文件，你可以解压后，找到 `抽号器.exe`，`右键-（显示更多选项）-发送到-桌面快捷方式` 来创建快捷方式
...
> 💡如果你不想配置姓名，仅配置座号范围，你可以查看下面 [手动配置座号范围](https://github.com/LuoYunXi0407/Rand#%E9%85%8D%E7%BD%AE%E5%BA%A7%E5%8F%B7%E8%8C%83%E5%9B%B4) 部分
...
> 你应该有类似于这样子的一个 Excel 表格，里面有班级学生名单，通常可以来自考勤表或者作业登记表甚至成绩单
...
4. （按需配置）如果某个座号没有对应的学生，你可以把它改为「【空】」，这样抽号的时候就会绕过这个座号
...
- [ ] 自定义启动时打开的页面
- [ ] 另外写一个悬浮窗软件，支持在 PowerPoint 等播放时唤起抽号器并提供其他快捷功能 ~~（之前肝了几个月被我硬盘格式化格掉了，闹）~~
...
- 绝大部分代码是我在大概 22-23 年堆的屎山，那个时候 LLM 还没有那么发达，我当时也没有把代码交给 LLM 审查或者重构，就摆着屎山了。有空一定重构！
- 因此，其实一开始是不打算开源的，因为我 ~~当时~~ 压根不会写软件 ~~（现在其实也不太会，只不过不懂的可以问 LLM 了）~~，~~写的代码只有上天能看得懂~~，但是看到 [Awesome-Iwb](https://github.com/Awesome-Iwb/Awesome-Iwb) 上的项目都是开源的，所以就把这无比屎山的代码端上来了，轻点喷，毕竟主要是能跑就行，代码屎山不重要了（
